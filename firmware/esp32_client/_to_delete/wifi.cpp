...
static void drawPasswordScreenChrome() {
  uint16_t kbTop = SCREEN_H - KB_H;
  display.fillRect(0, 0, SCREEN_W, kbTop - KB_GAP, COL_DARKGREY);
...
void wifiTick() {
  ...
  // Connection-result handling deliberately keeps running even if user
  // backed out of the WiFi screen mid-connect
...
void handleWifiTouch(bool isTouching, int16_t x, int16_t y) {
  ...
  // Swallow touches until a lift has occurred since this screen (or this
  // sub-screen's pending close) started
  if (!wifiArmed || wifiClosePending) return;
  if (wifiWasTouching) return;  // everything here is tap-to-act, not drag
  wifiWasTouching = true;

  // Close button lives in the same spot on every sub-screen.
  ...
  switch (wscreen) {
    ...
    case WSCR_CONNECTING:
      return;  // only close button is interactive here
